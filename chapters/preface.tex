...
\centering
\begin{tikzpicture}[
    tool/.style={draw=black!75, rounded corners=8pt, line width=1.0pt, minimum height=1.45cm, align=center, inner sep=5pt, font=\small},
    core/.style={draw=black!80, rounded corners=8pt, line width=1.0pt, minimum height=1.6cm, align=center, inner sep=5pt, font=\small},
    arrow/.style={->, >=stealth, line width=1.0pt}
]
    % 顶部两个工具
    \node[tool, fill=blue!6, draw=blue!65!black, text width=3.05cm] (complex) at (-2.8, 2.3)
    {\textbf{\large 复变函数}\\[4pt]解析函数、级数\\展开、留数};
    \node[tool, fill=green!6, draw=green!55!black, text width=3.05cm] (transform) at (2.8, 2.3)
    {\textbf{\large 积分变换}\\[4pt]傅里叶变换\\拉普拉斯变换};

    % 中间核心
    \node[core, fill=orange!12, draw=orange!70!black, text width=4.7cm] (pde) at (0, 0)
    {\textbf{\large 数理方程}\\[4pt]三类方程的建立与求解};

    % 底部产物
    \node[tool, fill=red!6, draw=red!55!black, text width=4.7cm, minimum height=1.45cm] (special) at (0, -2.35)
    {\textbf{\large 特殊函数}\\[4pt]勒让德函数、贝塞尔函数};
...
\begin{enumerate}
    ...
    \item \textbf{积分变换}：它并不是凭空把问题``变没''，而是换一种表示、换一个观察角度，把原来难以下手的微分方程问题化成更容易处理的代数方程。傅里叶变换和拉普拉斯变换，是后面会反复使用的重要工具。
