...
\[
    ...
    \end{cases}
\]
在 $z=0$ 处的连续性，并借此说明``连续''只要求极限等于函数值。
\end{lizi}
...
\begin{itemize}
    ...
    \item 前面连续性一节中的 $|z|^2$ 应在这里回扣，用同一个例子说明``连续''和``解析''不是一回事。
...
由上面的公式和基本求导法则推出：多项式在整个复平面内可导，有理函数在其定义域内可导。
\end{tuilun}
...
\begin{lizi}{用求导法则求有理函数的导数}{}
求
\[
    f(z)=\frac{z^2+1}{z-\mathrm{i}}
\]
在其定义域内的导数，并借此让学生熟悉加法法则与商法则的配合使用。
\end{lizi}
...
\[
    f(z)=|z|^2
\]
在 $z=0$ 处是否可导，并回扣前面连续性一节中同一个函数的例子，说明``连续''和``解析''不是一回事。
\end{lizi}
...
\begin{lizi}{用 $\bar z$ 偏导快速排除解析性}{}
计算
\[
    \bar z,\qquad z\bar z,\qquad \operatorname{Re}z
\]
的
\[
    \frac{\partial f}{\partial \bar z},
