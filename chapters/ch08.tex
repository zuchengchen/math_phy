...
\begin{tishi}[本节小结]
本节最重要的收获，不是多记了一个新公式，而是看清了傅里叶变换\index{傅里叶变换}的来历：周期越长，频率间隔越小；当周期趋于无穷时，离散谱自然过渡为连续谱，于是求和也自然过渡为积分。后面傅里叶变换和逆变换的定义，正是建立在这条极限链条上的。
\end{tishi}
...
\begin{tishi}[证明最后一步会用到什么]
为了不让反演公式最后一步显得像``突然跳到结论''，我们先把要用的结论单独说清楚。

若 $\phi(u)$ 在 $u=0$ 附近分段光滑，并且在整个实轴上绝对可积，那么有
\begin{equation}
    \lim_{A\to\infty}
    \frac{1}{\pi}
    \int_{-\infty}^{+\infty}
    \phi(u)\frac{\sin Au}{u}\,\mathrm{d}u
    =
    \frac{\phi(0+)+\phi(0-)}{2}.
\end{equation}
...
\[
    \frac{\sin Au}{\pi u}
\]
在 $A$ 很大时，会把函数在 $u=0$ 附近的信息``挤出来''；如果这里有跳跃，它提取的不是左边或右边单独一侧，而是两侧的平均值。这和傅里叶级数在间断点处的平均值结论，本质上是同一个机制。

在下面的证明里，我们将取
\[
    \phi(u)=f(x-u).
\]
由于 $f$ 在任意有限区间上分段光滑、并且绝对可积，所以这个结论恰好可以应用。
\end{tishi}
...
\begin{equation}
    ...
    =
    \frac{1}{\pi}\int_{-\infty}^{+\infty}
    f(x-u)\frac{\sin Au}{u}\,\mathrm{d}u.
\end{equation}
...
\begin{tishi}[本节小结]
本节真正要记住的是三件事：第一，傅里叶变换和逆变换是一对把时域与频域联系起来的基本公式；第二，绝对可积给出了一个够用的存在条件；第三，反演公式在间断点处恢复的是左右极限的平均值，而不是某一侧的值。
\end{tishi}
...
这里最容易卡住的一步，是为什么可以把积分路径从实轴平移到
\[
    \operatorname{Im} z=\frac{\omega}{2a}
\]
这条平行线。这个做法并不是凭空规定的，而是前面复变函数部分已经见过的一个典型思想：如果被积函数在所围成区域内解析，而且连接两条路径的侧边积分在极限下消失，那么两条路径上的积分就相等。

在这里，被积函数
\[
    \mathrm{e}^{-az^2}
\]
是整函数，在整个复平面内解析；另一方面，高斯因子在水平方向有很强的衰减，因此把矩形围道的左右两侧拉到无穷远时，侧边贡献会消失。于是实轴上的积分就可以平移到上方那条平行线上。

如果你暂时不想追究这一步的严格技术细节，也可以先把它理解成：\textbf{高斯函数衰减足够好，又没有奇点，所以这类积分路径可以平移。}这里和前面复积分、柯西定理的联系，正是复变函数知识在积分变换中的一次具体应用。
...
\begin{tishi}[本节小结]
这一节的任务，不是让你机械背下一串变换对，而是建立频谱直观：矩形函数对应 $\mathrm{sinc}$ 型振荡频谱，高斯函数变换后仍是高斯，奇偶性和实值性能帮助我们快速判断频谱结构。以后做题时，很多结果其实在动手算之前就应该先凭直觉猜到一个大致形状。
\end{tishi}
...
\begin{tishi}[本节小结]
本节把傅里叶变换\index{傅里叶变换}整理成了真正可用的工具箱。真正要形成的习惯是：看到平移、调制、伸缩、导数或乘以 $x$ 这类结构时，先想能否直接调用性质，而不是每次都从定义积分重新开始。后面用傅里叶变换解方程，靠的正是这种``先识别结构，再选性质''的眼光。
\end{tishi}
...
\begin{tishi}[本节小结]
这一节最核心的收获是：卷积在时域里看起来是积分运算，但到了频域就变成了普通乘法；而帕塞瓦尔等式又告诉我们，时域和频域描述的是同一份能量信息。也就是说，频域方法不是把问题``变花哨''了，而是在保留核心物理内容的前提下，把运算真正简化了。
\end{tishi}
...
设
\begin{equation}
    f(x)=\mathrm{e}^{-ax}, \qquad x\ge 0,\ a>0.
\end{equation}
求它的余弦变换和正弦变换，并说明它们分别对应什么延拓。

\begin{jie}
先看余弦变换。把 $f(x)$ 作偶延拓，得到
\begin{equation}
    f_e(x)=\mathrm{e}^{-a|x|}.
\end{equation}
第三节已经算过
\begin{equation}
    \mathcal{F}[\mathrm{e}^{-a|x|}](\omega)=\frac{2a}{a^2+\omega^2}.
\end{equation}
而对偶函数来说，
\begin{equation}
    \mathcal{F}[f_e](\omega)=2F_c(\omega),
\end{equation}
所以
\begin{equation}
    F_c(\omega)=\int_0^{+\infty}\mathrm{e}^{-ax}\cos \omega x\,\mathrm{d}x
    =
    \frac{a}{a^2+\omega^2}.
\end{equation}

再看正弦变换。把 $f(x)$ 作奇延拓，得到
\begin{equation}
    f_o(x)=
    \begin{cases}
        \mathrm{e}^{-ax}, & x\ge 0, \\
...
\begin{align}
    F_s(\omega)
    &=
    \int_0^{+\infty}\mathrm{e}^{-ax}\sin \omega x\,\mathrm{d}x \\
    &=
    \operatorname{Im}
    \int_0^{+\infty}\mathrm{e}^{-(a-\mathrm{i}\omega)x}\,\mathrm{d}x \\
    &=
    \operatorname{Im}
    \left(\frac{1}{a-\mathrm{i}\omega}\right)
    =
...
\begin{itemize}
    \item 余弦变换对应的是偶延拓；
    \item 正弦变换对应的是奇延拓；
    \item 同一个半轴函数，因为延拓方式不同，会自然导向两种不同的变换。
\end{itemize}
\end{jie}
\end{lizi}
...
\begin{tishi}[本节小结]
这一节真正要带走的结论是：半轴问题不是把整轴傅里叶变换机械照搬过去，而是要先看边界条件，再决定延拓方式。偶延拓自然导向余弦变换，奇延拓自然导向正弦变换；这就是为什么数学物理方程中的边界条件会直接影响我们选什么积分变换。
\end{tishi}
...
\begin{tishi}[本节小结]
本节最关键的体验是：对整条实轴上的热方程，傅里叶变换\index{傅里叶变换}会把空间微分变成频域乘法，于是偏微分方程被降成了常微分方程；再通过逆变换和卷积定理，就能回到热核表示。这正是积分变换法的基本工作模式：先变换简化，再逆变换回到原问题。
\end{tishi}
