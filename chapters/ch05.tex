...
留数的定义告诉我们：求留数本质上是在找洛朗展开里的 $c_{-1}$。但若奇点只是简单极点，其主要奇异部分恰好只有一项
\[
    \frac{c_{-1}}{z-z_0}.
\]
这时把 $(z-z_0)$ 乘上去，就能直接把这项``剥''出来。所以这一条公式的意义在于：它把``读洛朗系数''转化成了``算一个极限''。
...
\begin{tishi}[这条公式该怎样使用]
使用时最关键的一步，不是马上代极限，而是先确认 $z_0$ 的确是\textbf{一阶极点}。确认以后，通常按下面的思路做：
\begin{enumerate}
    \item 先把 $(z-z_0)$ 乘上去；
    \item 尽量先约掉会导致分母为零的因子；
    \item 最后再令 $z\to z_0$。
\end{enumerate}
如果一开始就没有先判断极点阶数，就很容易把这个公式误用到高阶极点上去。
\end{tishi}
...
上一条公式已经把简单极点的留数化成极限；对最常见的有理函数题，这个极限还可以再往前走一步，直接化成分子在该点的取值除以分母导数。它的意义是：在很多题里，求留数甚至连极限都不必真正去算。
...
当奇点不是简单极点时，前面的极限法就不够了，因为只乘一个 $(z-z_0)$ 还不能把全部负幂项消掉。这时我们需要一个新办法，把主部里的别的项都压下去，只把 $c_{-1}$ 单独留下来。高阶极点公式的意义，正是在于给出了这种``定点提取 $c_{-1}$''的方法。
...
\begin{tishi}[高阶极点公式怎么落到计算上]
这条公式看起来长，但真正操作时可以按三步走：
\begin{enumerate}
    \item 先确认 $z_0$ 是 $m$ 阶极点；
    \item 先把 $(z-z_0)^m f(z)$ 化简到尽量简单；
    \item 再求 $m-1$ 阶导数，并在最后代入 $z_0$。
\end{enumerate}
不过也要保持判断力：如果导数会越算越复杂，而函数本身又容易展开，那么直接作洛朗展开往往更透明。
\end{tishi}
...
\]
这类振荡因子时，情况会比单纯的有理函数稍复杂一些。这里最核心的问题是：\textbf{大半圆弧上的积分为什么还能消失？}回答这个问题的常用工具就是约当引理。它的意义就在于：把``指数振荡''和``半平面衰减''联系起来，从而给我们一个可以放心丢掉大圆弧的判据。
...
\]
现在把最后这个积分估计清楚。由于 $\sin\theta=\sin(\pi-\theta)$，并且 $\sin\theta$ 在区间 $[0,\pi/2]$ 上是凹函数，所以它的图像位于连接两端点 $(0,0)$ 与 $(\pi/2,1)$ 的弦上方，从而
\[
...
\begin{align*}
    ...
    &=
    2\int_0^{\pi/2} \mathrm{e}^{-aR\sin\theta}\,\mathrm{d}\theta \\
    &\le
    2\int_0^{\pi/2} \mathrm{e}^{-\frac{2aR}{\pi}\theta}\,\mathrm{d}\theta \\
    &=
    \frac{\pi}{aR}\left(1-\mathrm{e}^{-aR}\right)
    \le
    \frac{\pi}{aR}.
\end{align*}
于是
...
\begin{tishi}[使用约当引理前先检查什么]
这条引理不是看到 $\mathrm{e}^{\mathrm{i}ax}$ 就能直接套。更稳妥的检查顺序是：
\begin{enumerate}
    \item 指数因子在哪个半平面衰减。对 $\mathrm{e}^{\mathrm{i}az}$ 而言，若 $a>0$ 应选上半平面；若 $a<0$ 则应改选下半平面；
    \item 除了指数因子外，前面的 $f(z)$ 在大圆弧上是否至少满足最大模趋于零。
\end{enumerate}
只有这两点都成立，才可以有根据地说大圆弧积分消失。
\end{tishi}
...
\begin{tishi}[这条定理的意义]
这条定理最重要的地方在于：零点和极点的``重数''，到了
\[
    \frac{f'(z)}{f(z)}
\]
里会直接变成留数。于是原来不太容易整体累加的重数信息，现在就能被留数定理一次性收集起来。这正是后面辐角原理的出发点。
\end{tishi}

\begin{lizi}{先把重数转成留数}{}
设
\[
    f(z)=\frac{(z-1)^2}{(z+2)^3}.
\]
求
\[
    \frac{f'(z)}{f(z)}
...
\begin{jie}
函数 $f$ 在 $z=1$ 处有一个二重零点，在 $z=-2$ 处有一个三阶极点。由上面的定理，
\[
    \operatorname{Res}\left[\frac{f'(z)}{f(z)},1\right]=2,\qquad
    \operatorname{Res}\left[\frac{f'(z)}{f(z)},-2\right]=-3.
\]
这两个数并不是偶然算出来的，它们恰好就是``零点重数''和``极点重数取负号''。因此若某条围道 $C$ 同时包围这两个点，那么
\[
...
\begin{dingliyi}{对数留数}{}
设 $f$ 在简单闭曲线 $C$ 上不为零，并且在 $C$ 的内部除有限个零点和极点外解析，则
\[
...
\begin{tishi}[为什么要单独定义``对数留数'']
它本质上是把
\[
    \frac{f'(z)}{f(z)}
\]
在围道内的留数总和做了一次统一记账。单独给它起名字，是为了后面可以更紧凑地表述``零点总数减极点总数''这类结论。
\end{tishi}

\begin{dingli}{辐角原理}{}
设 $f$ 在简单闭曲线 $C$ 上不为零，并且在 $C$ 的内部除有限个极点外解析。若 $f$ 在 $C$ 内部有 $N$ 个零点和 $P$ 个极点，均按重数计，则
\[
    \frac{1}{2\pi \mathrm{i}}
    \oint_C \frac{f'(z)}{f(z)}\,\mathrm{d}z
    =
...
\begin{tishi}[辐角原理怎么使用]
真正做题时，建议按下面的顺序走：
\begin{enumerate}
    \item 先检查 $f$ 在围道 $C$ 上没有零点也没有极点；
    \item 再弄清 $C$ 内部有哪些极点，它们的重数是多少；
    \item 设法计算
    \[
        \oint_C \frac{f'(z)}{f(z)}\,\mathrm{d}z
    \]
    或直接计算其留数和；
    \item 最后由 $N-P$ 的值反推出零点个数。若围道内没有极点，就直接得到了零点总重数。
\end{enumerate}
\end{tishi}
...
\begin{lizi}{先由 $N-P$ 读出信息}{}
设
\[
    f(z)=\frac{(z-1)^2}{(z+1)(z-3)}.
\]
求
\[
    \frac{1}{2\pi \mathrm{i}}
    \oint_{|z|=2}\frac{f'(z)}{f(z)}\,\mathrm{d}z.
\]

\begin{jie}
先检查边界条件。在圆周 $|z|=2$ 上，函数 $f$ 没有零点也没有极点，因此可以应用辐角原理。
...
\[
    ...
    1.
\]
这个结果的含义是：在该围道内部，零点总重数比极点总重数多 $1$。
\end{jie}
\end{lizi}

\subsection{鲁歇定理}

辐角原理告诉我们怎样把零点个数转化成积分，但很多时候直接去算
\[
    \oint \frac{f'(z)}{f(z)}\,\mathrm{d}z
\]
并不轻松。鲁歇定理的意义就在于：如果在边界上有一个部分明显占主导，那么内部零点个数就不会因为较小的扰动而改变。这样，我们就能把难函数的数根问题转移给一个更容易数根的主项。
...
\begin{tishi}[鲁歇定理最常见的用法]
真正难的地方通常不是证明不等式，而是\textbf{先选谁来当主项}。比较稳妥的思路是：
\begin{enumerate}
    \item 先在给定边界上找一个模长最容易估计、零点个数又容易数的主项；
    \item 把剩余部分全部并入另一个函数；
    \item 再验证边界上是否成立 $|f|>|g|$。
\end{enumerate}
一旦这一步成立，内部零点个数就可以直接从主项读出来。
\end{tishi}
