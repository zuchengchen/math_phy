...
如果你现在暂时合上书，最好已经能顺着下面这条线口头复述一遍：复积分为什么必须先从参数化和曲线出发，原函数为什么会让积分只由端点决定，柯西积分定理怎样把这种整齐结构推广到单连通区域里的解析函数，多连通区域里的``洞''又会怎样改变积分行为，而柯西积分公式又怎样把边界积分直接连到函数值和各阶导数上。本章的知识主线如下图所示：
...
\node[foundation, right=of a] (b) {原函数与\\复积分基本公式};
\node[foundation, right=of b] (c) {单连通与多连通\\的区域差异};

\node[pivot, below=1.30cm of b] (d) {柯西积分定理};
\node[branch, below left=1.35cm and 1.05cm of d] (e) {路径无关\\与原函数存在};
\node[branch, below right=1.35cm and 1.05cm of d] (f) {典型闭路积分\\与 $2\pi\mathrm{i}$};
...
读这张图时，可以把自己想象成正在复盘一整章课：先学会怎样沿曲线真正去算复积分，再通过原函数把积分和端点联系起来，接着用柯西积分定理解释为什么解析函数在单连通区域里会突然变得路径无关；而一旦区域里有洞，积分行为又会立刻变样。最后，所有这些内容都会汇聚到柯西积分公式那里，并继续长出高阶导数公式、解析函数的刚性，以及下一章的泰勒展开。只要这条线是顺的，本章就算真正学通了。
...
\begin{enumerate}
    ...
    \[
        z=z(t)
    \]
    把它化成实变量积分来计算。真正做题时，先把路径参数化清楚，往往就已经完成了一半。
