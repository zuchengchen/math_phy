...
在 $R \neq 0$ 处，函数只依赖于径向变量 $R$。二维极坐标下
\begin{equation}
    \nabla^2 f(R) = \frac{1}{R}\frac{\mathrm{d}}{\mathrm{d}R}\left(R\frac{\mathrm{d}f}{\mathrm{d}R}\right)
\end{equation}
因此
\begin{equation}
    \nabla^2 \ln R = \frac{1}{R}\frac{\mathrm{d}}{\mathrm{d}R}\left(R \cdot \frac{1}{R}\right) = 0
\end{equation}
所以在 $R \neq 0$ 处，
\begin{equation}
    \nabla^2\left(-\frac{1}{2\pi}\ln R\right) = 0
\end{equation}

现在考察奇点处。取以 $\boldsymbol{r}'$ 为圆心、半径为 $\epsilon$ 的小圆盘 $B_\epsilon$，由平面上的散度定理得
\begin{equation}
    \int_{B_\epsilon} \nabla^2\left(-\frac{1}{2\pi}\ln R\right)\,\mathrm{d}A
    = \oint_{\partial B_\epsilon} \frac{\partial}{\partial n}\left(-\frac{1}{2\pi}\ln R\right)\,\mathrm{d}s
\end{equation}
在边界 $\partial B_\epsilon$ 上，$R = \epsilon$，且 $\frac{\partial \ln R}{\partial n} = \frac{1}{\epsilon}$，于是
\begin{equation}
    \oint_{\partial B_\epsilon} \frac{\partial}{\partial n}\left(-\frac{1}{2\pi}\ln R\right)\,\mathrm{d}s
    = -\frac{1}{2\pi}\oint_{\partial B_\epsilon} \frac{1}{\epsilon}\,\mathrm{d}s
    = -\frac{1}{2\pi}\int_0^{2\pi} \frac{1}{\epsilon}\cdot \epsilon\,\mathrm{d}\theta
    = -1
\end{equation}

因此
\begin{equation}
    \nabla^2\left(-\frac{1}{2\pi}\ln|\boldsymbol{r} - \boldsymbol{r}'|\right) = -\delta(\boldsymbol{r} - \boldsymbol{r}')
\end{equation}
\end{proof}

\begin{wulibj}[为什么二维基本解是对数型]
三维点电荷的电势随 $1/R$ 衰减；而二维问题常常可以看作``沿垂直纸面方向均匀延伸的线源截面''，其场强大小更像 $1/R$，再对径向积分就得到 $\ln R$。这就是为什么二维拉普拉斯方程的基本解不是 $1/R$，而是对数函数。
\end{wulibj}
...
对于上半平面 $y > 0$，边界条件为 $G|_{y=0} = 0$。

使用\textbf{镜像法}：把源点 $\boldsymbol{r}' = (x', y')$ 关于 $x$ 轴反射到镜像点 $\boldsymbol{r}'' = (x', -y')$，并在镜像点放一个负源。

格林函数\index{格林函数}为
\begin{equation}
    \boxed{G(\boldsymbol{r}, \boldsymbol{r}') = -\frac{1}{2\pi}\ln\frac{|\boldsymbol{r} - \boldsymbol{r}'|}{|\boldsymbol{r} - \boldsymbol{r}''|}}
\end{equation}

这是因为当边界点满足 $y = 0$ 时，它到原点 $\boldsymbol{r}'$ 与镜像点 $\boldsymbol{r}''$ 的距离相等，所以 $G = 0$。

\begin{lizi}{}{}
求上半平面 $y > 0$ 内拉普拉斯方程的解，边界条件为 $u(x, 0) = f(x)$。

\begin{jie}
二维情形的边界积分表示与三维完全类似，只是基本解换成了二维的对数型基本解。于是
\begin{equation}
    u(x', y') = -\int_{-\infty}^{+\infty} f(x)\frac{\partial G}{\partial n}\Big|_{y=0}\,\mathrm{d}x
\end{equation}
其中边界外法向指向下方，因此 $\frac{\partial}{\partial n} = -\frac{\partial}{\partial y}$。

由上面的格林函数可得
\begin{equation}
    \frac{\partial G}{\partial n}\Big|_{y=0} = -\frac{1}{\pi}\frac{y'}{(x-x')^2 + y'^2}
\end{equation}

代入后得到
\begin{equation}
    \boxed{u(x', y') = \frac{1}{\pi}\int_{-\infty}^{+\infty} \frac{y' f(x)}{(x-x')^2 + y'^2}\,\mathrm{d}x}
\end{equation}
...
\begin{tishi}[和前面各章的分工]
对于二维势场问题，矩形域、圆域等规则区域常用分离变量法；上半平面等无界区域常用傅里叶变换\index{傅里叶变换}法或格林函数\index{格林函数}法；如果边界可以通过解析映射化成标准区域，那么保角映射也是非常有力的工具。把这些方法放在一起看，学生就更容易明白：它们不是互相割裂的技巧，而是在处理同一类物理模型时各有擅长。
\end{tishi}
